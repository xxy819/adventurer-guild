import sys
sys.path.insert(0, 'D:/adventurer-guild')
...
with app.app_context():
    ...
    for data in all_tasks:
        ...
        level = data['level']
        # B级及以上用admin发布（官方），其他用demo
        pub = admin_user if level in ['B', 'A', 'S', 'SS', 'SSS'] else demo_user
        task = Task(
            title=data['title'],
            level=level,
    ...
    print(f'任务数据已更新！新增 {new_count} 个，当前共 {total} 个任务')
    print('账号: demo / demo123  或  admin / admin123')
